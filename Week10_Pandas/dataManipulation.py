...
def getSeriesOfUnique(dataFrame, nameOfCol, delim = '/'):
    ...
    for valueInCol in valuesWithDelims:
        ...
        valueAsList = re.split('/', valueInCol)                        # returns names seperated by a / 
        uniqueValues.update(valueAsList)                               # adds the list into the set. Only unique values within a set
    ds = pd.Series(list(uniqueValues))                                 # new panda series
...
def randomiseDataOnList(df, oldList):                            # https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.replace.html#pandas.DataFrame.replace
    newList = oldList.copy()
    random.shuffle(newList)
    #print(oldList) # debug checking they are different
    #print(newList) # debug
    df.replace(oldList, newList, inplace=True, regex=True)      # inplace - do it n the same df o a new one / regex  - yes or no
